function balanced() {
  ...
  function createModuleMethod(data, name, json) {
    ...
    function _makeCall(uriOptions, options, callback) {
      ...
      var check = checkRequiredFields(newData.requires, options, json, name);
      if(check.error === true) {
      ...
      new Request($scope.requestOptions).request(newData.path, newData.method, options, function(err, result) {
        callback(err, result);
      });
    }
